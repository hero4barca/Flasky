...
from flask import render_template, redirect, url_for, flash, abort, request, make_response, current_app
from . import main
from .. import db
...
def unfollow(username):
    ...
    if not current_user.is_following(user):
        flash('You are not currently following this user.')
        return redirect(url_for('.user', username=username))
    current_user.unfollow(user)
    db.session.commit()
    flash('You have successfuly unfollowed %s.' % username)
    return redirect(url_for('.user', username=username))
...
def follows(username):
    ...
    return render_template('followers.html', user=user, title="Followed by", endpoint='.follows',
                                                pagination=pagination,  follows=following)
